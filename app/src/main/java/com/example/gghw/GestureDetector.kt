...
class GlassGestureDetector(context: Context, onGestureListener: OnGestureListener) {
    ...
    private fun detectGesture(): Boolean {
        ...
        val tan =
            if (firstFingerDistanceX != 0f) abs(firstFingerDistanceY / firstFingerDistanceX).toDouble() else Double.Companion.MAX_VALUE

        if (isTwoFingerGesture) {
            val tanSecondFinger =
                if (secondFingerDistanceX != 0f) abs(secondFingerDistanceY / secondFingerDistanceX).toDouble() else Double.Companion.MAX_VALUE
    ...
    companion object {
        ...
        const val SWIPE_DISTANCE_THRESHOLD_PX: Int = 100
        const val SWIPE_VELOCITY_THRESHOLD_PX: Int = 100
    }
}